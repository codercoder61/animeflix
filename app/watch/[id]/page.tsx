  ...
  const fetchEpisodes = async () => {
    try {
      ...
      const response = await fetch(
        `https://essentials-rounds-impacts-traditions.trycloudflare.com/getAnimeEpisodesInfo?episodeHref=https://anime3rb.com/episode/${animeId}/1&page=${currentPage}`
      );
...
const fetchEpisodeSource = async (number=1) => {
      try {
        setIsLoading(true)
        const response = await fetch(
          `https://essentials-rounds-impacts-traditions.trycloudflare.com/getEpisodeSource?episodeHref=https://anime3rb.com/episode/${animeId}/${number}`
        )
        const data = await response.json()
        if (response.ok) {
  ...
  useEffect(()=>{
    const fetchAnimeInfo = async () => {
      try {
        setIsLoading(true)
        const response = await fetch(
          `https://essentials-rounds-impacts-traditions.trycloudflare.com/getAnimeInfo?animeId=${animeId}`
        )
        const data = await response.json()
